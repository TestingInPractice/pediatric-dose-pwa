...
## Backlog идей (НЕ разрабатывать без явного запроса)

### 💊 Напоминания и график приёма таблеток

> Зафиксировано как идея 2026-08-23. В работу не брать до отдельного решения владельца.

**Суть:**

1. После расчёта и подтверждения приёма — возможность завести **периодический график приёма** препарата (например, каждые 24 часа).
2. График заводится через **календарь + заведение будильника**.
3. **Эскалация напоминаний:** если человек не подтверждает приём — напомнить ещё 2 раза.
4. Если подтверждения нет после всех напоминаний → **отправить сообщение доверенному человеку** через это же приложение.
5. Для пункта 4 нужна **связь аккаунтов** (пользователь ↔ доверенный человек) — новая сущность в модели данных.

**Технические заметки:**
- Расширение существующего модуля `js/reminders.js` (Reminder.checkOverdue уже есть, Notification API используется).
- Новые сущности IndexedDB: `schedules` (график), `reminder_log` (история напоминаний), `trusted_contacts` (доверенные лица).
- ⚠️ Проект сейчас **Zero backend** — доставка сообщения доверенному человеку требует либо бэкенда/push-сервера, либо обходных путей (Telegram-бот, Web Share API). Нужен ресёрч до старта.
- Открытые вопросы: настраиваемый интервал? несколько препаратов в одном графике? поведение при офлайне?
